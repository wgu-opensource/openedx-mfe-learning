...
After doing changes to https://github.com/wgu-opensource/frontend-app-learning ulmo branch, you need to build a dist in order to make it usable via npm.

To do that, follow the following process:

1. Change to branch ulmo-dist
2. Pull changes from ulmo
3. Run a build
4. Force-add the dist folder and commit
5. Push changes to ulmo-dist
...
git checkout ulmo-dist
git pull origin ulmo
make build
git add -f dist
git commit -m "Release <your release>"
git push origin ulmo-dist
